...
define(function(require) {
  ...
  function generateCommonTab() {
    ...
    if (allowScroll) {
      ...
      if (toggleMore) {
        $header.append('<div class="mip-vd-tabs-nav-toggle"><img src=' +
          ICON_SRC + '></div>');
      }
    } else {
      $header
  ...
  function generateEpisodeDown(linkTpl) {
    ...
    wrapper.append(
      tabList.map(function(v, index) {
        var epFragment = '<div class="' + CONTENT_CLS +
          ' mip-vd-tabs-episode-content" ' + (index === tabCurNum ? '' :
            'style="display:none;" ') + ' >';
        for (var j = v.from; j <= v.to; j++) {
          ...
          epFragment = epFragment +
            '<span class="mip-vd-tabs-episode-item ' + selectedClass +
            '"' + link + '>' + j + '</span>';
        }
        epFragment += '</div>';
        return epFragment;
    ...
    if (tabCount > 1) {
      ...
      tabFragment += tabList.map(function(v, index) {
        ...
        return '<li class="' + ITEM_CLS + ' ' + selectedClass + '">' +
          v.text + '</li>';
      }).join('');
      tabFragment += '</ul>';
      if (tabCount > scrollNum) {
  ...
  function generateWrapper() {
    ...
    var totalNum = parseInt($el.attr('total'), 10) || 1;
    if (totalNum > 4) {
      $result = $('<div class="' + VIEW_CLS + '">' + '<ul class="' +
        NAV_CLS + '"></ul>' + '</div>'
      );
    } else {
      $result = $('<div class="mip-vd-tabs-row-tile">' + '<ul class="' +
        NAV_CLS + '"></ul>' + '</div>'
      );
    }
    return $result;
  ...
  function generateToggle($result) {
    ...
    $result.append('<div class="' + TOGGLE_CLS + '">' + '<img src=' +
      ICON_SRC + '>' + '</div>');
    return $result;
  }
  ...
  function generateEpisode($result, total, current, textTpl, linkTpl,
    ...
    for (var i = Math.max(1, currentNum - EPISODE_RANGE),
      ...
      html = html + '<a class="' + ITEM_CLS + ' ' + (i === currentNum ?
          SELECTED_CLS : '') + '" ' + (linkTpl ? ' href="' + linkTpl.replace(
          TPL_REG, i) + '"' : '') + '>' + tpl.replace(TPL_REG, '' + i) +
        '</a>';
    }
    ...
    (function register(ptr) {
      var _this = tab;
      var $navLayer = $('<div class="mip-vd-tabs-nav-layer"><p>' + _this.toggleLabel +
        '</p></div>');
