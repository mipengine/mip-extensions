...
define(function(require) {
    ...
    var jibFunction = function() {
        ...
        $("#item_hd_form form").on("submit", function() {
            var textVal = $.trim($("#item_so_keyword").val()), srcType = $("#item_so_keyword").attr("src_type");
            if (textVal == "帮您寻医问药") {
                textVal = "";
            } else {
                textVal = textVal;
        ...
        $(".simulate-sel").on({
            click: function() {
                var self = $(this), selBox = self.parent().find(".pro-selectDiv");
                if (self.hasClass("on")) {
                    self.removeClass("on");
                    selBox.hide();
                } else {
        ...
        $(".pro-selectDiv").on({
            click: function(e) {
                var self = $(this), target = $(e.target), selectBox = self.parent().find(".simulate-sel"), showTxt = selectBox.find("span");
                txt = target.text(), ill_id = selectBox.attr("accesskey"), illid = selectBox.attr("ill-id"), 
                area_id = target.attr("value");
                showTxt.text(txt);
                self.hide();
                ...
                if (self.hasClass("zhuanjia")) {
                    $.ajax({
                        ...
                        success: function(data) {
                            if (data !== "error") {
                                var list = "", lLen = data.data.length > 3 ? 3 : data.data.length;
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if (data !== "error") {
                                var docList = "", lLen = data.data.length > 3 ? 3 : data.data.length;
                                for (var i = 0; i < lLen; i++) {
        ...
        function showErrorMessage(a, b, c) {
            ...
            $("body").append('<section id="showMessage" style="display:none;position: fixed;z-index: 99;top:' + b + 'px;left: 50%;background: rgba( 0 , 0 , 0 ,.8);color: #fff;margin: -15px 0 0 -100px;width: 200px;height: 30px;line-height: 30px;border-radius: 5px;box-shadow: 0 1px 0 1px rgba(0,0,0,.2);text-align: center; font-size:14px;">' + a + "</section>");
            $("#showMessage").show();
            setTimeout(function() {
                $("#showMessage").hide();
                $("#showMessage").remove();
