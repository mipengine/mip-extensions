...
[notice] 页面中如果有视频时，请注意亮点：一、不要将视频作为背景填充，因为在 android 中视频播放器优先级太高会遮住页面，导致用户操作不能正常进行。二、视频作为每个段落内容时，不建议设置自动播放，即添加 `autoplay` 属性，因为 iOS 下的部分浏览器会直接弹出系统的视频播放器，影响用户体验。如果需要使用视频，可以暂时使用 GIF + background-audio 进行设置。
