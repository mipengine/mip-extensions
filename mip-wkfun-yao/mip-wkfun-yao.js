...
define(function(require){
    ...
    $("#menuShow").on("click", function() {
        var height = $(document).height(), height1 = $(".site-navigation").height(), node = $('<div id="menuMask" class="mask-layer" style="height:' + height + 'px;"></div>');
        $("body").append(node);
        $(".mask").animate({
            translate3D: "0, 0, 0"
        }, 200);
    ...
    function showErrorMessage(a, b, c) {
        ...
        $("body").append('<section id="showMessage" style="  width: 100%;position: fixed;top: ' + b + "px;left: " + c + 'px;z-index:300;"><div style="display:-webkit-box;-webkit-box-orient: horizontal;-webkit-box-pack: center;display: -moz-box;-moz-box-orient: horizontal;-moz-box-pack: center;display: -o-box;-o-box-orient: horizontal;-o-box-pack: center;display: -ms-box;-ms-box-orient: horizontal;-ms-box-pack: center;display: box;box-orient: horizontal;box-pack: center;width:100%;"><div  style="background: rgba( 0 , 0 , 0 ,.8);  color: #fff;  padding: 0 10px;  height: 30px;  line-height: 30px;  border-radius: 5px;  box-shadow: 0 1px 0 1px rgba(0,0,0,.2);  text-align: center;  font-size: 14px;">' + a + "</div></div></section>");
        $("#showMessage").show();
        setTimeout(function() {
            $("#showMessage").hide();
            $("#showMessage").remove();
    ...
    function nums() {
        $(".js-tab span").on("click", function() {
            ...
            var policy_id = $(this).data("policyid"), policy_price = $(this).data("policyprice"), policy_name = $(this).data("policyname");
        ...
        $(".js-num-plus").on("click", function() {
            var self = $(this), minus = $(".js-num-minus"), input = $(".js-num-text"), num = parseInt(input.val());
            if (num === 1) {
        ...
        $(".js-num-minus").on("click", function() {
            var self = $(this), plus = $(".js-num-plus"), input = $(".js-num-text"), num = parseInt(input.val());
            if (num === 999) {
    ...
    //新建地址
    var regName = /^[\u4e00-\u9fa5]{1,30}$/, regPhone = /^13[0-9]{9}$|15[0-9]{9}$|18[0-9]{9}$|14[579][0-9]{8}$|17[0678][0-9]{8}$/, regCode = /^[1-9]\d{5}(?!\d)/, //邮编
    regsitexq = /^[\u4E00-\u9FA5A-Za-z0-9]+$/;
    //详细地址
    //保存地址
    $("#siteForm").on("submit", function() {
        var uName = $.trim($(".js-u-name").val()), uPhone = $.trim($(".js-u-phone").val()), city = $.trim($("#value1").val());
        //三级地区值
        sitexq = $.trim($(".js-u-sitexq").val()), postcode = $.trim($(".js-postcode").val());
        var area = $("#value1").val();
        var strs = [];
        //定义一数组
        strs = area.split(",");
    ...
    $(".m-manage-site li .lf-wrapper").on("click", function() {
        if ($(this).parents("li").hasClass("cur")) {
            ...
            var address_id = $(this).parents("li").find(".address_id").val();
            var user_id = $(this).parents("li").find(".user_id").val();
            user_address_mrdz(address_id);
        }
    });
    /*地址左滑删除*/
    var moveW = parseInt($(".rg-wrapper").eq(0).css("width"));
    ...
    $(".del-site").each(function() {
        $(this).on("click", function() {
            if (confirm("你确定要删除吗？")) {
                ...
                user_address_del(address_id, user_id);
                return;
            }
        });
    });
    function user_address_del(id, user_id) {
        if (id != "") {
            $.ajax({
                type: "POST",
    ...
    function swiper() {
        ...
        $(".cart-wrapper").each(function() {
            $(this).on("swipeleft", function() {
                $(this).addClass("selected").parents(".cart-order-item").siblings().find(".cart-wrapper").removeClass("selected");
            });
            $(this).on("swiperight", function() {
                $(this).removeClass("selected");
            });
    ...
    $(".demand-pop-btn").on("click", function() {
        if ($(".js-tab span").hasClass("current")) {
            var policy_id = $(".js-tab .current").data("policyid");
            var policy_name = $(".js-tab .current").data("policyname");
            var policy_price = $(".js-tab .current").data("policyprice");
            $("#policy_id").val(policy_id);
            $("#policy_name").val(policy_name);
            $("#policy_price").val(policy_price);
        }
    });
    var UA = navigator.userAgent.toLowerCase(), CLICK = "click";
    //cpc
    ...
    $.ajax({
        ...
        success: function(data) {
            $(".p-bj-load").remove();
            var standard = data.standard.list, spackings = data.spackings, standardLength = data.standard.total;
            if (spackings) {
            ...
            //取得包装
            var con = '<div class="bzlist pr">' + '<p class="list-t"><a class="check-more box-flex-1 bt" href="javascript:;">选择包装：<span class="list-t-text">' + spackings[0].name + '</span><span class="list-btn"></span></a></p>' + '<div id="wrapper" class="wrapper-box pa none"><div id="scroller"><ul>';
            ...
            $.each(standard, function(i, t) {
                if (i >= 5) {
                    con += '<li top-price="' + t.top_price + '" class="none"><a class="dis-box" href="' + t.href + '"><p class="box-flex-1 cost-name">' + t.store_name + '</p><p class="pr10"><b class="cost-num"><em>￥</em>' + t.shop_price + '</b><span class="cost-icon"><b></b></span></p></a></li>';
                } else {
                    con += '<li top-price="' + t.top_price + '"><a class="dis-box" href="' + t.href + '"><p class="box-flex-1 cost-name">' + t.store_name + '</p><p class="pr10"><b class="cost-num"><em>￥</em>' + t.shop_price + '</b><span class="cost-icon"><b></b></span></p></a></li>';
                }
            });
            con += "</ul>";
            if (standardLength > 5) {
    ...
    //查看更多报价	
    var bjMaker = true;
    $("#cpcBox").on(CLICK, ".check-more-num", function() {
        if (!bjMaker) {
            return;
        }
        bjMaker = false;
    ...
    var timer = null, ajaxMaker = true;
    $("#cpcBox").on(CLICK, ".wrapper-box li", function() {
        if (!ajaxMaker) {
            return;
        }
        ajaxMaker = false;
        clearTimeout(timer);
        var self = $(this), text = self.text(), uuidbox = self.attr("uuidBox");
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data.list, function(i, t) {
                    if (i >= 5) {
                        con += '<li class="none"><a class="dis-box" href="' + t.href + '"><p class="box-flex-1 cost-name">' + t.store_name + '</p><p class="pr10"><b class="cost-num"><em>￥</em>' + t.shop_price + '</b><span class="cost-icon"><b></b></span></p></a></li>';
                    } else {
                        con += '<li><a class="dis-box" href="' + t.href + '"><p class="box-flex-1 cost-name">' + t.store_name + '</p><p class="pr10"><b class="cost-num"><em>￥</em>' + t.shop_price + '</b><span class="cost-icon"><b></b></span></p></a></li>';
                    }
                });
                con += "</ul>";
                if (length > 5) {
    ...
    $("#loopFocus").on(CLICK, function() {
        var height = $(document).height(), mask = '<div id="pBigMask" class="mask-layer" style="height:' + height + 'px;"></div>';
        $("body").append(mask);
        $("#pBigShow").css({
            width: "100%",
            height: height + "px"
    ...
    if (GetQueryString("lon") && GetQueryString("lat")) {
        //APP进来的访客
        var longitude = GetQueryString("lon"), latitude = GetQueryString("lat");
        $.ajax({
            type: "get",
    ...
    customElem.prototype.build = function() {
        ...
        loadJs(elem, "https://scs.static.xywy.com/tools/iscroll.js", function() {
            ...
            $("#cpcBox").on(CLICK, ".check-more", function() {
