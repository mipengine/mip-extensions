...
define(function (require) {
    ...
    var down = {
        ...
        mrtit: function () {
            ...
            var n = i.attr('href');
            var c = $('.ca');
            var r = [631, 632, 633, 634, 636, 694, 695, 696, 697,
            700, 727, 728, 729, 730, 731, 732, 733, 734, 735, 797,
            798, 799, 800, 801, 806];
            var o = [830, 831, 832, 833, 834, 835, 836, 837, 838,
            839, 840, 841, 813, 814, 815, 816, 817, 818, 819, 820,
            821, 822, 823, 824, 825, 826, 827, 828, 802, 803, 804,
            805, 807, 808, 809, 810, 758, 759, 760, 761, 762, 763,
            764, 765, 766, 767, 768, 769, 770, 771, 673, 672, 674,
            675, 676, 677, 678, 679, 680, 681, 682, 683, 684, 685,
            686, 687, 688, 689, 690, 618, 619, 620, 621, 622, 623,
            624, 625, 591, 592, 593, 594, 595, 596, 597, 598, 637,
            638, 711, 712, 713, 714, 842, 465, 466, 467, 468, 469,
            470, 471, 472, 473, 476, 477, 386, 387, 388, 389, 390,
            391, 414, 415, 416, 417, 571, 572, 573, 574, 422, 285,
            282, 284, 286, 287, 288, 535, 616, 508, 384, 299, 304,
            345, 478, 479, 480, 482, 506, 298, 561, 562, 629, 557];
        ...
        titTab: function () {
            if ($('.xgwz ul li').length > 0) {
                ...
                $('.xgwz').hide(), $('#tab span').eq(1).hide();
            }
            $('#tab span').click(function () {
                if ('简介' === $(this).text()) {
                    $(this).addClass('cur').siblings().removeClass('cur');
                    $('.xyc,.cont,#comment,.xgwz,.similar,#tcsyy,.guess,#xgk,.historyver').show();
                    $('.tips_more,.h5online,.hot_gamerec,.rank,.rela_down').show();
                }
                else if ('教程' === $(this).text()) {
                    $(this).addClass('cur').siblings().removeClass('cur');
                    $('.cont,.xyc,.similar,#tcsyy,#xgk,.guess,.historyver').hide();
                    $('.tips_more,.h5online,.hot_gamerec,.rank').hide();
                    $('.xgwz,#comment,.rela_down').show();
                }
                else if ('评论' === $(this).text()) {
                    $(this).addClass('cur').siblings().removeClass('cur');
                    $('.xgwz,.cont,.xyc,.similar,#tcsyy,#xgk,.guess,.historyver').hide(), $('#comment').show();
                    $('.tips_more,.h5online,.hot_gamerec,.rank,.rela_down').hide(), $('#comment').show();
                }
        ...
        touchSlide: function (oclass) {
            ...
            if (0 !== n.length) {
                ...
                var l = parseInt($(window).width() - 16, 10);
                var c = {s: [], d: ''};
                var u = 0;
                o.width(l), $('#tags-main').width(l);
                for (var d = 1; d < o.length; d++) {
                    a += '<span></span>';

                }
                if ($(oclass).find('.pagenum').html(a),
                r.width(o.length * o.width()), s.eq(0).addClass('active'), s.click(function () {
                        var t = $(this).index();
                        ...
                        c.s[1] = t.targetTouches[0].pageY, c.s[2] = (new Date()).getTime();
                    }, !1), r[0].addEventListener('touchmove', function (t) {
                        var a = Math.abs(t.targetTouches[0].pageX - c.s[0]);
                        if (a >= Math.abs(t.targetTouches[0].pageY - c.s[1]) && '' === c.d) {
                            c.d = 1;
                        }
                        else if ('' === c.d) {
                            c.d = 0;
                        ...
                        if (1 === c.d) {
                            var pa = -(l * u - t.targetTouches[0].pageX + c.s[0]);
                            t.preventDefault(), r.css({
                                '-webkit-transform': 'translate3d(' + pa + 'px, 0px, 0px)'
                            });
                        }
                    ...
                    $('#wrapper2')[0].addEventListener('touchend', function (t) {
                        i();
                    }, !1);
                }

                if ($('#wrapper4').length > 0) {
                    $('#wrapper4')[0].addEventListener('touchend', function (t) {
                        i();
                    }, !1);
                }
            }
            function t(t) {
                ...
                i(), $(oclass).find('.pagenum span').eq(u).addClass('active').siblings().removeClass('active'),
                $('.guess-nav li').eq(u).addClass('active').siblings().removeClass('active');
            }
            function i() {
                setTimeout(function () {
        ...
        tcsyy: function () {
            if ($('#tcsyy').length > 0) {
                ...
                if (len === 0) {
                    ...
                    if (len < 8) {
                        $('#tcsyy').find('.pagenum').remove();
                    }

                    for (var i = 0; i < page; i++) {
                        var html = '<div class="tags-main-box"><ul class="tags-box-ul"></ul></div>';
                        $('#tcsyy .tags-main-ul > li').slice(0, 8).wrapAll(html);
                    }
                    if ($('#tags-main1').length > 0) {
        ...
        h5: function () {
            var li = $('.h5online .tags-main-ul > li');
            var len = li.length;
            var page = Math.ceil(len / 8);
            for (var i = 0; i < page; i++) {
                var htm = '<div class="tags-main-box"><ul class="tags-box-ul"></ul></div>';
                $('.h5online .tags-main-ul > li').slice(0, 8).wrapAll(htm);
            }
            if ($('#tags-main2').length > 0) {
                this.touchSlide('#h5online');
            }
        },
        reladown: function () {
            $('.rela_down').addClass('pictxt').removeClass('list');
            var li = $('.rela_down .tags-main-ul > li');
            var len = li.length;
            var page = Math.ceil(len / 8);
            for (var i = 0; i < page; i++) {
                var html = '<div class="tags-main-box"><ul class="tags-box-ul"></ul></div>';
                $('.rela_down .tags-main-ul > li').slice(0, 8).wrapAll(html);
            }
            if ($('#tags-main3').length > 0) {
        ...
        footer: function () {
            $('.mip_footer ul li.arrow').click(function () {
                var t = $(this);
                var o = $('.mip_footer');
                t.toggleClass('on');
                if (t.hasClass('on')) {
        ...
        init: function () {
            this.mrtit(), this.xfNav(), this.titTab(), this.downHref(), this.footer(), this.hotRec(),
            this.touchSlide(), this.show(), this.historyver(), this.tcsyy(), this.reladown(), this.h5(),
            this.focuswe(), this.dif();
        }
    };
