...
其中**需要注意的是**，在开发组件中，如果需要书写额外 CSS 样式时，官方提供了一种机制，该机制允许开发者在组件目录中下构建 setting 文件夹，并生成 example.preset 的文件，同时将样式写入其中，在这些工作完成后通过工具开启本地预览服务就可以将样式自动加入到预览页面中，如 [mip-accordion](https://github.com/mipengine/mip-extensions/tree/master/src/mip-accordion/setting) 组件。
